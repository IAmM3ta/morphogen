...
const emptyCwd = () => mkdtempSync(join(tmpdir(), "grok-og-empty-"));
...
test("site title beats a preview grok.me slug in the manifest", () => {
  const manifest = JSON.parse(
    renderWebManifest("cap-granite-lagoon-rocket.grok.me", { title: "MORPHOS" }),
  );
  assert.equal(manifest.name, "MORPHOS");
  assert.equal(manifest.short_name, "MORPHOS");
});

test("workspace site.json names the live manifest MORPHOS", () => {
