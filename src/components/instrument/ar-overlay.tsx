...
import { lerpQuad, quadMatrix3d, type Pt } from "@/lib/morphogen/homography";
...
export function ArOverlay({
  ...
  useEffect(() => {
    ...
    const loop = async () => {
      if (field && field.width > 0) {
        if (copy.width !== field.width || copy.height !== field.height) {
      ...
      if (det && video.readyState >= 2) {
        try {
          ...
          if (hit && corners && corners.length === 4) {
            ...
            const mapped = mapCorners(corners as [Pt, Pt, Pt, Pt], video, view, host);
  ...
  return (
    <div ref={hostRef} className="absolute inset-0 z-50 bg-bg" data-ui>
      <video ref={videoRef} className="absolute inset-0 size-full object-cover" playsInline muted autoPlay />
      <canvas
        ref={copyRef}
        className="pointer-events-none absolute left-0 top-0 origin-top-left mix-blend-screen"
      ...
      <div className="absolute inset-x-0 top-0 flex items-start justify-between px-4 pt-[max(1rem,env(safe-area-inset-top))]">
        <p className="max-w-[70%] font-mono text-[10px] tracking-[0.16em] text-fg uppercase">
          {live ? `Living${maker ? ` · ${maker}` : ""}` : "Look for a MORPHOS sticker"}
        </p>
